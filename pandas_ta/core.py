...
class Strategy:
    ...
    def __post_init__(self):
        ...
        if self.ta is None:
            ...
            is_ta = all([(isinstance(_, dict) and len(_.keys()) > 0) for _ in
                self.ta])
        else:
            s = (
...
AllStrategy = Strategy(name='All', description=
    'All the indicators with their default settings. Pandas TA default.',
    ta=None)


CommonStrategy = Strategy(name='Common Price and Volume SMAs', description=
    'Common Price SMAs: 10, 20, 50, 200 and Volume SMA: 20.', ta=[{'kind':
    'sma', 'length': 10}, {'kind': 'sma', 'length': 20}, {'kind': 'sma',
    'length': 50}, {'kind': 'sma', 'length': 200}, {'kind': 'sma', 'close':
    'volume', 'length': 20, 'prefix': 'VOL'}])
...
class BasePandasObject(PandasObject):
    ...
    def __init__(self, df, **kwargs):
        if df.empty:
            return
        if len(df.columns) > 0:
            common_names = {'Date': 'date', 'Time': 'time', 'Timestamp':
                'timestamp', 'Datetime': 'datetime', 'Open': 'open', 'High':
                'high', 'Low': 'low', 'Close': 'close', 'Adj Close':
                'adj_close', 'Volume': 'volume', 'Dividends': 'dividends',
                'Stock Splits': 'split'}
...
class AnalysisIndicators(BasePandasObject):
    ...
    def _validate(obj: Tuple[pd.DataFrame, pd.Series]):
        if not isinstance(obj, pd.DataFrame) and not isinstance(obj, pd.Series
            ...
            raise AttributeError(
                '[X] Must be either a Pandas Series or DataFrame.')

    def __call__(self, kind: str=None, timed: bool=False, version: bool=
        False, **kwargs):
        if version:
            print(
                f'Pandas TA - Technical Analysis Indicators - v{self.version}')
        try:
    ...
    def _add_prefix_suffix(self, result=None, **kwargs) ->None:
        ...
        if result is None:
            ...
            if isinstance(result, pd.Series):
                ...
                result.columns = [(prefix + column + suffix) for column in
                    result.columns]
    ...
    def _append(self, result=None, **kwargs) ->None:
        ...
        if 'append' in kwargs and kwargs['append']:
            ...
            if df is None or result is None:
                ...
                simplefilter(action='ignore', category=pd.errors.
                    PerformanceWarning)
                if 'col_names' in kwargs and not isinstance(kwargs[
                    'col_names'], tuple):
                    kwargs['col_names'] = kwargs['col_names'],
                if isinstance(result, pd.DataFrame):
                    if 'col_names' in kwargs and isinstance(kwargs[
                        'col_names'], tuple):
                        if len(kwargs['col_names']) >= len(result.columns):
                            for col, ind_name in zip(result.columns, kwargs
                                ['col_names']):
                    ...
                    ind_name = kwargs['col_names'][0
                        ] if 'col_names' in kwargs and isinstance(kwargs[
                        'col_names'], tuple) else result.name
                    df[ind_name] = result
    ...
    def _get_column(self, series):
        ...
        if isinstance(series, pd.Series):
            ...
            if series in df.columns:
                ...
                NOT_FOUND = (
                    f"[X] Ooops!!! It's {series not in df.columns}, the series '{series}' was not found in {cols}"
                    )
        ...
        * Appends the result to main DataFrame
        ...
        if not isinstance(result, (pd.Series, pd.DataFrame)):
            ...
            result = result.iloc[:, [int(n) for n in kwargs['col_numbers']]
                ...
                ) and 'col_numbers' in kwargs and kwargs['col_numbers'
    ...
    def indicators(self, **kwargs):
        ...
        ta_properties = ['adjusted', 'categories', 'cores',
            'datetime_ordered', 'exchange', 'last_run', 'reverse', 'ticker',
            'time_range', 'to_utc', 'version']
        ta_indicators = list(x for x in dir(pd.DataFrame().ta) if not x.
            startswith('_') and not x.endswith('_'))
...
Candle Patterns:
    ...
    def strategy(self, *args, **kwargs):
        ...
        excluded = ['above', 'above_value', 'below', 'below_value', 'cross',
            'cross_value', 'long_run', 'short_run', 'td_seq', 'tsignals',
            'vp', 'xsignals']
        ...
        if use_multiprocessing and mode['custom']:
            has_col_names = True if len([(True) for x in ta if 'col_names' in
                x and isinstance(x['col_names'], tuple)]) else False
            if has_col_names:
        ...
        if use_multiprocessing:
            ...
            with Pool(self.cores) as pool:
                _chunksize = (mp_chunksize - 1 if mp_chunksize > _total_ta else
                    int(npLog10(_total_ta)) + 1)
                if verbose:
                    print(
                ...
                results = None
                if mode['custom']:
                    custom_ta = [(ind['kind'], ind['params'] if 'params' in
                        ind and isinstance(ind['params'], tuple) else (), {
                        **ind, **kwargs}) for ind in ta]
                    results = pool.imap(self._mp_worker, custom_ta, _chunksize)
                else:
                    default_ta = [(ind, tuple(), kwargs) for ind in ta]
                    if all_ordered:
                        if Imports['tqdm']:
                            results = tqdm(pool.imap(self._mp_worker,
                                default_ta, _chunksize))
                        else:
                            results = pool.imap(self._mp_worker, default_ta,
                                _chunksize)
                    elif Imports['tqdm']:
                        results = tqdm(pool.imap_unordered(self._mp_worker,
                            default_ta, _chunksize))
                    else:
                        results = pool.imap_unordered(self._mp_worker,
                            default_ta, _chunksize)
                if results is None:
            ...
            if mode['custom']:
                if Imports['tqdm'] and verbose:
                    ...
                    for ind in pbar:
                        params = ind['params'
                            ] if 'params' in ind and isinstance(ind[
                            'params'], tuple) else tuple()
    ...
    def cdl_pattern(self, name='all', offset=None, **kwargs):
        ...
        result = cdl_pattern(open_=open_, high=high, low=low, close=close,
            name=name, offset=offset, **kwargs)
    ...
    def cdl_z(self, full=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = cdl_z(open_=open_, high=high, low=low, close=close, full=
            full, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def ha(self, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = ha(open_=open_, high=high, low=low, close=close, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)

    def ebsw(self, close=None, length=None, bars=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = ebsw(close=close, length=length, bars=bars, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def ao(self, fast=None, slow=None, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        result = ao(high=high, low=low, fast=fast, slow=slow, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)

    def apo(self, fast=None, slow=None, mamode=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = apo(close=close, fast=fast, slow=slow, mamode=mamode,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def bias(self, length=None, mamode=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = bias(close=close, length=length, mamode=mamode, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def bop(self, percentage=False, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = bop(open_=open_, high=high, low=low, close=close,
            percentage=percentage, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def brar(self, length=None, scalar=None, drift=None, offset=None, **kwargs
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = brar(open_=open_, high=high, low=low, close=close, length=
            length, scalar=scalar, drift=drift, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def cci(self, length=None, c=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = cci(high=high, low=low, close=close, length=length, c=c,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def cmo(self, length=None, scalar=None, drift=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = cmo(close=close, length=length, scalar=scalar, drift=drift,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def coppock(self, length=None, fast=None, slow=None, offset=None, **kwargs
        ...
        close = self._get_column(kwargs.pop('close', 'close'))
        result = coppock(close=close, length=length, fast=fast, slow=slow,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def dm(self, drift=None, offset=None, mamode=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        result = dm(high=high, low=low, drift=drift, mamode=mamode, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)

    def er(self, length=None, drift=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = er(close=close, length=length, drift=drift, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def eri(self, length=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = eri(high=high, low=low, close=close, length=length, offset
            =offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def inertia(self, length=None, rvi_length=None, scalar=None, refined=
        None, thirds=None, mamode=None, drift=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        if refined is not None or thirds is not None:
            high = self._get_column(kwargs.pop('high', 'high'))
            low = self._get_column(kwargs.pop('low', 'low'))
            result = inertia(close=close, high=high, low=low, length=length,
                rvi_length=rvi_length, scalar=scalar, refined=refined,
                thirds=thirds, mamode=mamode, drift=drift, offset=offset,
                **kwargs)
        else:
            result = inertia(close=close, length=length, rvi_length=
                rvi_length, scalar=scalar, refined=refined, thirds=thirds,
    ...
    def kdj(self, length=None, signal=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = kdj(high=high, low=low, close=close, length=length, signal
            =signal, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def kst(self, roc1=None, roc2=None, roc3=None, roc4=None, sma1=None,
        sma2=None, sma3=None, sma4=None, signal=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = kst(close=close, roc1=roc1, roc2=roc2, roc3=roc3, roc4=
            roc4, sma1=sma1, sma2=sma2, sma3=sma3, sma4=sma4, signal=signal,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def macd(self, fast=None, slow=None, signal=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = macd(close=close, fast=fast, slow=slow, signal=signal,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def pgo(self, length=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = pgo(high=high, low=low, close=close, length=length, offset
            =offset, **kwargs)
        return self._post_process(result, **kwargs)

    def ppo(self, fast=None, slow=None, scalar=None, mamode=None, offset=
        None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = ppo(close=close, fast=fast, slow=slow, scalar=scalar,
            mamode=mamode, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def psl(self, open_=None, length=None, scalar=None, drift=None, offset=
        None, **kwargs):
        if open_ is not None:
            open_ = self._get_column(kwargs.pop('open', 'open'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = psl(close=close, open_=open_, length=length, scalar=scalar,
            drift=drift, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def pvo(self, fast=None, slow=None, signal=None, scalar=None, offset=
        None, **kwargs):
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = pvo(volume=volume, fast=fast, slow=slow, signal=signal,
            scalar=scalar, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def qqe(self, length=None, smooth=None, factor=None, mamode=None,
        offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = qqe(close=close, length=length, smooth=smooth, factor=
            factor, mamode=mamode, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def rsi(self, length=None, scalar=None, drift=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = rsi(close=close, length=length, scalar=scalar, drift=drift,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def rsx(self, length=None, drift=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = rsx(close=close, length=length, drift=drift, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def rvgi(self, length=None, swma_length=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = rvgi(open_=open_, high=high, low=low, close=close, length=
            length, swma_length=swma_length, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def smi(self, fast=None, slow=None, signal=None, scalar=None, offset=
        None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = smi(close=close, fast=fast, slow=slow, signal=signal,
            scalar=scalar, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def squeeze(self, bb_length=None, bb_std=None, kc_length=None,
        kc_scalar=None, mom_length=None, mom_smooth=None, use_tr=None,
        mamode=None, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = squeeze(high=high, low=low, close=close, bb_length=
            bb_length, bb_std=bb_std, kc_length=kc_length, kc_scalar=
            kc_scalar, mom_length=mom_length, mom_smooth=mom_smooth, use_tr
            =use_tr, mamode=mamode, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def squeeze_pro(self, bb_length=None, bb_std=None, kc_length=None,
        kc_scalar_wide=None, kc_scalar_normal=None, kc_scalar_narrow=None,
        mom_length=None, mom_smooth=None, use_tr=None, mamode=None, offset=
        None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = squeeze_pro(high=high, low=low, close=close, bb_length=
            bb_length, bb_std=bb_std, kc_length=kc_length, kc_scalar_wide=
            kc_scalar_wide, kc_scalar_normal=kc_scalar_normal,
            kc_scalar_narrow=kc_scalar_narrow, mom_length=mom_length,
            mom_smooth=mom_smooth, use_tr=use_tr, mamode=mamode, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)

    def stc(self, ma1=None, ma2=None, osc=None, tclength=None, fast=None,
        slow=None, factor=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = stc(close=close, ma1=ma1, ma2=ma2, osc=osc, tclength=
            tclength, fast=fast, slow=slow, factor=factor, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)

    def stoch(self, fast_k=None, slow_k=None, slow_d=None, mamode=None,
        offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = stoch(high=high, low=low, close=close, fast_k=fast_k,
            slow_k=slow_k, slow_d=slow_d, mamode=mamode, offset=offset, **
            kwargs)
        return self._post_process(result, **kwargs)

    def stochrsi(self, length=None, rsi_length=None, k=None, d=None, mamode
        =None, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = stochrsi(high=high, low=low, close=close, length=length,
            rsi_length=rsi_length, k=k, d=d, mamode=mamode, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)

    def td_seq(self, asint=None, offset=None, show_all=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = td_seq(close=close, asint=asint, offset=offset, show_all=
            show_all, **kwargs)
        return self._post_process(result, **kwargs)

    def trix(self, length=None, signal=None, scalar=None, drift=None,
        offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = trix(close=close, length=length, signal=signal, scalar=
            scalar, drift=drift, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def tsi(self, fast=None, slow=None, drift=None, mamode=None, offset=
        None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = tsi(close=close, fast=fast, slow=slow, drift=drift, mamode
            =mamode, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def uo(self, fast=None, medium=None, slow=None, fast_w=None, medium_w=
        None, slow_w=None, drift=None, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = uo(high=high, low=low, close=close, fast=fast, medium=
            medium, slow=slow, fast_w=fast_w, medium_w=medium_w, slow_w=
            slow_w, drift=drift, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def willr(self, length=None, percentage=True, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = willr(high=high, low=low, close=close, length=length,
            percentage=percentage, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def alma(self, length=None, sigma=None, distribution_offset=None,
        offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = alma(close=close, length=length, sigma=sigma,
            distribution_offset=distribution_offset, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def hilo(self, high_length=None, low_length=None, mamode=None, offset=
        None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = hilo(high=high, low=low, close=close, high_length=
            high_length, low_length=low_length, mamode=mamode, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def jma(self, length=None, phase=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = jma(close=close, length=length, phase=phase, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)

    def kama(self, length=None, fast=None, slow=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = kama(close=close, length=length, fast=fast, slow=slow,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def ichimoku(self, tenkan=None, kijun=None, senkou=None, include_chikou
        =True, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result, span = ichimoku(high=high, low=low, close=close, tenkan=
            tenkan, kijun=kijun, senkou=senkou, include_chikou=
            include_chikou, offset=offset, **kwargs)
    ...
    def linreg(self, length=None, offset=None, adjust=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = linreg(close=close, length=length, offset=offset, adjust=
            adjust, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def midprice(self, length=None, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        result = midprice(high=high, low=low, length=length, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def ohlc4(self, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = ohlc4(open_=open_, high=high, low=low, close=close, offset
            =offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def ssf(self, length=None, poles=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = ssf(close=close, length=length, poles=poles, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def supertrend(self, length=None, multiplier=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = supertrend(high=high, low=low, close=close, length=length,
            multiplier=multiplier, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def vwap(self, anchor=None, offset=None, **kwargs):
        ...
        result = vwap(high=high, low=low, close=close, volume=volume,
            anchor=anchor, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def vwma(self, volume=None, length=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = vwma(close=close, volume=volume, length=length, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def zlma(self, length=None, mamode=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = zlma(close=close, length=length, mamode=mamode, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)

    def log_return(self, length=None, cumulative=False, percent=False,
        offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = log_return(close=close, length=length, cumulative=
            cumulative, percent=percent, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def percent_return(self, length=None, cumulative=False, percent=False,
        offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = percent_return(close=close, length=length, cumulative=
            cumulative, percent=percent, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def entropy(self, length=None, base=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = entropy(close=close, length=length, base=base, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def quantile(self, length=None, q=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = quantile(close=close, length=length, q=q, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def tos_stdevall(self, length=None, stds=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = tos_stdevall(close=close, length=length, stds=stds, offset
            =offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def zscore(self, length=None, std=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = zscore(close=close, length=length, std=std, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)

    def adx(self, length=None, lensig=None, mamode=None, scalar=None, drift
        =None, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = adx(high=high, low=low, close=close, length=length, lensig
            =lensig, mamode=mamode, scalar=scalar, drift=drift, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)

    def amat(self, fast=None, slow=None, mamode=None, lookback=None, offset
        =None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = amat(close=close, fast=fast, slow=slow, mamode=mamode,
            lookback=lookback, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def aroon(self, length=None, scalar=None, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        result = aroon(high=high, low=low, length=length, scalar=scalar,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def chop(self, length=None, atr_length=None, scalar=None, drift=None,
        offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = chop(high=high, low=low, close=close, length=length,
            atr_length=atr_length, scalar=scalar, drift=drift, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def cksp(self, p=None, x=None, q=None, mamode=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = cksp(high=high, low=low, close=close, p=p, x=x, q=q,
            mamode=mamode, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def decay(self, length=None, mode=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = decay(close=close, length=length, mode=mode, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)

    def decreasing(self, length=None, strict=None, asint=None, offset=None,
        **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = decreasing(close=close, length=length, strict=strict,
            asint=asint, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def dpo(self, length=None, centered=True, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = dpo(close=close, length=length, centered=centered, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)

    def increasing(self, length=None, strict=None, asint=None, offset=None,
        **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = increasing(close=close, length=length, strict=strict,
            asint=asint, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def long_run(self, fast=None, slow=None, length=None, offset=None, **kwargs
        ...
        if fast is None and slow is None:
            ...
            result = long_run(fast=fast, slow=slow, length=length, offset=
                offset, **kwargs)
            return self._post_process(result, **kwargs)
    ...
    def psar(self, af0=None, af=None, max_af=None, offset=None, **kwargs):
        ...
        result = psar(high=high, low=low, close=close, af0=af0, af=af,
            max_af=max_af, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def qstick(self, length=None, offset=None, **kwargs):
        open_ = self._get_column(kwargs.pop('open', 'open'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = qstick(open_=open_, close=close, length=length, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)

    def short_run(self, fast=None, slow=None, length=None, offset=None, **
        kwargs):
        if fast is None and slow is None:
            return self._df
        else:
            result = short_run(fast=fast, slow=slow, length=length, offset=
                offset, **kwargs)
            return self._post_process(result, **kwargs)

    def supertrend(self, period=None, multiplier=None, mamode=None, drift=
        None, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = supertrend(high=high, low=low, close=close, period=period,
            multiplier=multiplier, mamode=mamode, drift=drift, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)

    def tsignals(self, trend=None, asbool=None, trend_reset=None,
        trend_offset=None, offset=None, **kwargs):
        if trend is None:
            return self._df
        else:
            result = tsignals(trend, asbool=asbool, trend_offset=
                trend_offset, trend_reset=trend_reset, offset=offset, **kwargs)
            return self._post_process(result, **kwargs)
    ...
    def ttm_trend(self, length=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = ttm_trend(high=high, low=low, close=close, length=length,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def vhf(self, length=None, drift=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = vhf(close=close, length=length, drift=drift, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def vortex(self, drift=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = vortex(high=high, low=low, close=close, drift=drift,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def xsignals(self, signal=None, xa=None, xb=None, above=None, long=None,
        asbool=None, trend_reset=None, trend_offset=None, offset=None, **kwargs
        ):
        if signal is None:
            return self._df
        else:
            result = xsignals(signal=signal, xa=xa, xb=xb, above=above,
                long=long, asbool=asbool, trend_offset=trend_offset,
                trend_reset=trend_reset, offset=offset, **kwargs)
            return self._post_process(result, **kwargs)

    def above(self, asint=True, offset=None, **kwargs):
        a = self._get_column(kwargs.pop('close', 'a'))
        b = self._get_column(kwargs.pop('close', 'b'))
        result = above(series_a=a, series_b=b, asint=asint, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)

    def above_value(self, value=None, asint=True, offset=None, **kwargs):
        a = self._get_column(kwargs.pop('close', 'a'))
        result = above_value(series_a=a, value=value, asint=asint, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def below(self, asint=True, offset=None, **kwargs):
        a = self._get_column(kwargs.pop('close', 'a'))
        b = self._get_column(kwargs.pop('close', 'b'))
        result = below(series_a=a, series_b=b, asint=asint, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)

    def below_value(self, value=None, asint=True, offset=None, **kwargs):
        a = self._get_column(kwargs.pop('close', 'a'))
        result = below_value(series_a=a, value=value, asint=asint, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def cross(self, above=True, asint=True, offset=None, **kwargs):
        a = self._get_column(kwargs.pop('close', 'a'))
        b = self._get_column(kwargs.pop('close', 'b'))
        result = cross(series_a=a, series_b=b, above=above, asint=asint,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def cross_value(self, value=None, above=True, asint=True, offset=None,
        **kwargs):
        a = self._get_column(kwargs.pop('close', 'a'))
        result = cross_value(series_a=a, value=value, above=above, asint=
            asint, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def aberration(self, length=None, atr_length=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = aberration(high=high, low=low, close=close, length=length,
            atr_length=atr_length, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def accbands(self, length=None, c=None, mamode=None, offset=None, **kwargs
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = accbands(high=high, low=low, close=close, length=length, c
            =c, mamode=mamode, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def atr(self, length=None, mamode=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = atr(high=high, low=low, close=close, length=length, mamode
            =mamode, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def bbands(self, length=None, std=None, mamode=None, offset=None, **kwargs
        ...
        close = self._get_column(kwargs.pop('close', 'close'))
        result = bbands(close=close, length=length, std=std, mamode=mamode,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def donchian(self, lower_length=None, upper_length=None, offset=None,
        **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        result = donchian(high=high, low=low, lower_length=lower_length,
            upper_length=upper_length, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def hwc(self, na=None, nb=None, nc=None, nd=None, scalar=None, offset=
        None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = hwc(close=close, na=na, nb=nb, nc=nc, nd=nd, scalar=scalar,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def kc(self, length=None, scalar=None, mamode=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = kc(high=high, low=low, close=close, length=length, scalar=
            scalar, mamode=mamode, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def massi(self, fast=None, slow=None, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        result = massi(high=high, low=low, fast=fast, slow=slow, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def natr(self, length=None, mamode=None, scalar=None, offset=None, **kwargs
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = natr(high=high, low=low, close=close, length=length,
            mamode=mamode, scalar=scalar, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def pdist(self, drift=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = pdist(open_=open_, high=high, low=low, close=close, drift=
            drift, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def rvi(self, length=None, scalar=None, refined=None, thirds=None,
        mamode=None, drift=None, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = rvi(high=high, low=low, close=close, length=length, scalar
            =scalar, refined=refined, thirds=thirds, mamode=mamode, drift=
            drift, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def thermo(self, long=None, short=None, length=None, mamode=None, drift
        =None, offset=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        result = thermo(high=high, low=low, long=long, short=short, length=
            length, mamode=mamode, drift=drift, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def true_range(self, drift=None, offset=None, **kwargs):
        ...
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        result = true_range(high=high, low=low, close=close, drift=drift,
            offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def ui(self, length=None, scalar=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        result = ui(close=close, length=length, scalar=scalar, offset=
            offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def ad(self, open_=None, signed=True, offset=None, **kwargs):
        ...
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = ad(high=high, low=low, close=close, volume=volume, open_=
            open_, signed=signed, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def adosc(self, open_=None, fast=None, slow=None, signed=True, offset=
        None, **kwargs):
        if open_ is not None:
            open_ = self._get_column(kwargs.pop('open', 'open'))
        high = self._get_column(kwargs.pop('high', 'high'))
        ...
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = adosc(high=high, low=low, close=close, volume=volume,
            open_=open_, fast=fast, slow=slow, signed=signed, offset=offset,
            **kwargs)
        return self._post_process(result, **kwargs)

    def aobv(self, fast=None, slow=None, mamode=None, max_lookback=None,
        min_lookback=None, offset=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = aobv(close=close, volume=volume, fast=fast, slow=slow,
            mamode=mamode, max_lookback=max_lookback, min_lookback=
            min_lookback, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def cmf(self, open_=None, length=None, offset=None, **kwargs):
        ...
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = cmf(high=high, low=low, close=close, volume=volume, open_=
            open_, length=length, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)

    def efi(self, length=None, mamode=None, offset=None, drift=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = efi(close=close, volume=volume, length=length, offset=
            offset, mamode=mamode, drift=drift, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def eom(self, length=None, divisor=None, offset=None, drift=None, **kwargs
        ...
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = eom(high=high, low=low, close=close, volume=volume, length
            =length, divisor=divisor, offset=offset, drift=drift, **kwargs)
        return self._post_process(result, **kwargs)

    def kvo(self, fast=None, slow=None, length_sig=None, mamode=None,
        offset=None, drift=None, **kwargs):
        high = self._get_column(kwargs.pop('high', 'high'))
        low = self._get_column(kwargs.pop('low', 'low'))
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = kvo(high=high, low=low, close=close, volume=volume, fast=
            fast, slow=slow, length_sig=length_sig, mamode=mamode, offset=
            offset, drift=drift, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def mfi(self, length=None, drift=None, offset=None, **kwargs):
        ...
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = mfi(high=high, low=low, close=close, volume=volume, length
            =length, drift=drift, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def nvi(self, length=None, initial=None, signed=True, offset=None, **kwargs
        ...
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = nvi(close=close, volume=volume, length=length, initial=
            initial, signed=signed, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def pvi(self, length=None, initial=None, signed=True, offset=None, **kwargs
        ...
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = pvi(close=close, volume=volume, length=length, initial=
            initial, signed=signed, offset=offset, **kwargs)
        return self._post_process(result, **kwargs)
    ...
    def vp(self, width=None, percent=None, **kwargs):
        close = self._get_column(kwargs.pop('close', 'close'))
        volume = self._get_column(kwargs.pop('volume', 'volume'))
        result = vp(close=close, volume=volume, width=width, percent=
            percent, **kwargs)
        return self._post_process(result, **kwargs)
