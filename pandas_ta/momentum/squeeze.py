...
def squeeze(high, low, close, bb_length=None, bb_std=None, kc_length=None,
    kc_scalar=None, mom_length=None, mom_smooth=None, use_tr=None, mamode=
    ...
    kch = kc(high, low, close, length=kc_length, scalar=kc_scalar, mamode=
        mamode, tr=use_tr)
    ...
    data = {squeeze.name: squeeze, f'SQZ_ON': squeeze_on.astype(int) if
        asint else squeeze_on, f'SQZ_OFF': squeeze_off.astype(int) if asint
         else squeeze_off, f'SQZ_NO': no_squeeze.astype(int) if asint else
        no_squeeze}
