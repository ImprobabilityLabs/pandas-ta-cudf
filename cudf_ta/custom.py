...
import cudf
import cudf_ta
from cudf_ta import AnalysisIndicators
...
def import_dir(path, verbose=True):
    ...
    for d in dirs:
        ...
        # only look in directories which are valid cudf_ta categories
        if dirname not in [*cudf_ta.Category]:
            if verbose:
                print(f"[i] Skipping the sub-directory '{dirname}' since it's not a valid cudf_ta category.")
            continue
        ...
        for module in glob(abspath(join(path, dirname, "*.py"))):
            ...
            # figure out which of the modules functions to bind to cudf_ta
            ...
            if module_name not in cudf_ta.Category[dirname]:
                cudf_ta.Category[dirname].append(module_name)
...
import_dir.__doc__ = \
"""
Import a directory of custom indicators into cudf_ta
...
together with the existing cudf_ta functions just like if they were part of
cudf_ta.

If you at some late point would like to push them into the cudf_ta library
...
available in categories under the cudf_ta-folder. The only difference will
...
like all other native indicators in cudf_ta, including help functions.
"""
