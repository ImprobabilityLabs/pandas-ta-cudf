import cudf
from cuml.metrics import rsi
from cuml.preprocessing import ma
...
def qqe(close, length=None, smooth=None, factor=None, mamode=None, drift=None, offset=None, **kwargs):
    ...
    qqe_long = cudf.Series(float('nan'), index=close.index)
    qqe_short = cudf.Series(float('nan'), index=close.index)
    ...
    for i in range(1, m):
        ...
        if p_rsi > c_long and c_rsi > c_long:
            long.iloc[i] = max(c_long, lowerband.iloc[i])
        else:
            long.iloc[i] = lowerband.iloc[i]

        # Short Line
        if p_rsi < c_short and c_rsi < c_short:
            short.iloc[i] = min(c_short, upperband.iloc[i])
        else:
