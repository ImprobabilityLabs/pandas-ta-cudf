...
data = {'zero': [0, 0], 'a': [0, 1], 'b': [1, 0], 'c': [1, 1], 'crossed': [
    0, 1]}
...
class TestUtilities(TestCase):
    ...
    def test_above_value(self):
        ...
        npt.assert_array_equal(result, self.crosseddf['c'])
        result = self.utils.above_value(self.crosseddf['a'], self.crosseddf
            ['zero'])
    ...
    def test_below_value(self):
        ...
        npt.assert_array_equal(result, self.crosseddf['b'])
        result = self.utils.below_value(self.crosseddf['a'], self.crosseddf
            ['zero'])
        self.assertIsNone(result)
    ...
    def test_cross_below(self):
        result = self.utils.cross(self.crosseddf['b'], self.crosseddf['a'],
            above=False)
        self.assertIsInstance(result, Series)
        npt.assert_array_equal(result, self.crosseddf['crossed'])
    ...
    def test_df_dates(self):
        ...
        result = self.utils.df_dates(self.data, ['1999-11-01', '2020-08-15',
    ...
    def test_fibonacci(self):
        self.assertIs(type(self.utils.fibonacci(zero=True, weighted=False)),
            np.ndarray)
        npt.assert_array_equal(self.utils.fibonacci(zero=True), np.array([0,
            1, 1]))
        npt.assert_array_equal(self.utils.fibonacci(zero=False), np.array([
            1, 1]))
        npt.assert_array_equal(self.utils.fibonacci(n=0, zero=True,
            weighted=False), np.array([0]))
        npt.assert_array_equal(self.utils.fibonacci(n=0, zero=False,
            weighted=False), np.array([1]))
        npt.assert_array_equal(self.utils.fibonacci(n=5, zero=True,
            weighted=False), np.array([0, 1, 1, 2, 3, 5]))
        npt.assert_array_equal(self.utils.fibonacci(n=5, zero=False,
            weighted=False), np.array([1, 1, 2, 3, 5]))

    def test_fibonacci_weighted(self):
        self.assertIs(type(self.utils.fibonacci(zero=True, weighted=True)),
            np.ndarray)
        npt.assert_array_equal(self.utils.fibonacci(n=0, zero=True,
            weighted=True), np.array([0]))
        npt.assert_array_equal(self.utils.fibonacci(n=0, zero=False,
            weighted=True), np.array([1]))
        npt.assert_allclose(self.utils.fibonacci(n=5, zero=True, weighted=
            True), np.array([0, 1 / 12, 1 / 12, 1 / 6, 1 / 4, 5 / 12]))
        npt.assert_allclose(self.utils.fibonacci(n=5, zero=False, weighted=
            True), np.array([1 / 12, 1 / 12, 1 / 6, 1 / 4, 5 / 12]))
    ...
    def test_pascals_triangle(self):
        ...
        npt.assert_array_equal(self.utils.pascals_triangle(n=-5, weighted=
            True), array_5w)
        npt.assert_array_equal(self.utils.pascals_triangle(n=-5, weighted=
            True, inverse=True), array_5iw)
        npt.assert_array_equal(self.utils.pascals_triangle(n=5), array_5)
        npt.assert_array_equal(self.utils.pascals_triangle(n=5, weighted=
            True), array_5w)
        npt.assert_array_equal(self.utils.pascals_triangle(n=5, weighted=
    ...
    def test_symmetric_triangle(self):
        ...
        npt.assert_array_equal(self.utils.symmetric_triangle(n=4, weighted=
            True), array_4w)
        array_5 = self.utils.symmetric_triangle(n=5)
        array_5w = array_5 / np.sum(array_5)
        npt.assert_array_equal(self.utils.symmetric_triangle(n=5), array_5)
        npt.assert_array_equal(self.utils.symmetric_triangle(n=5, weighted=
            True), array_5w)
    ...
    def test_zero(self):
        self.assertEqual(self.utils.zero(-1e-16), 0)
        self.assertEqual(self.utils.zero(0), 0)
        self.assertEqual(self.utils.zero(0.0), 0)
        self.assertEqual(self.utils.zero(1e-16), 0)
        self.assertNotEqual(self.utils.zero(-1e-15), 0)
        self.assertNotEqual(self.utils.zero(1e-15), 0)
