...
class TestVolatilityExtension(TestCase):
    ...
    def test_aberration_ext(self):
        self.data = self.data.ta.aberration(append=True)
    ...
    def test_true_range_ext(self):
        self.data = self.data.ta.true_range(append=True)
        self.assertIsInstance(self.data, cudf.DataFrame)
        self.assertEqual(self.data.columns[-1], "TRUERANGE_1")

    def test_ui_ext(self):
        self.data = self.data.ta.ui(append=True)
        self.assertIsInstance(self.data, cudf.DataFrame)
        self.assertEqual(self.data.columns[-1], "UI_14")

        self.data = self.data.ta.ui(append=True, everget=True)
        self.assertIsInstance(self.data, cudf.DataFrame)
