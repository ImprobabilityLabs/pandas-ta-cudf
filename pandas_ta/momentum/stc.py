...
def stc(close, tclength=None, fast=None, slow=None, factor=None, offset=None, **kwargs):
    ...
    if isinstance(ma1, cudf.Series) and isinstance(ma2, cudf.Series) and not osc:
    ...
    elif isinstance(osc, cudf.Series):
        ...
        fastma = cudf.Series(ema(close, length=fast))
        slowma = cudf.Series(ema(close, length=slow))
        xmacd = fastma - slowma
        pff, pf = schaff_tc(close, xmacd, tclength, factor)
    stc = cudf.Series(pff, index=close.index)
    macd = cudf.Series(xmacd, index=close.index)
    stoch = cudf.Series(pf, index=close.index)
    if offset != 0:
