...
class Watchlist(object):
    ...
    def __init__(self, tickers: list, tf: str=None, name: str=None,
    ...
    def _init_data_source(self, ds: str) ->None:
        ...
        AVkwargs = {'api_key': 'YOUR API KEY', 'clean': True, 'export': 
            True, 'output_size': 'full', 'premium': False}
    ...
    def _drop_columns(self, df: pd.DataFrame, cols: list=None) ->pd.DataFrame:
        if cols is None or not isinstance(cols, list):
            cols = ['Unnamed: 0', 'date', 'split', 'split_coefficient',
                'dividend', 'dividends']
        else:
            cols
    ...
    def _load_all(self, **kwargs) ->dict:
        ...
        if self.tickers is not None and isinstance(self.tickers, list) and len(
            self.tickers):
            self.data = {ticker: self.load(ticker, **kwargs) for ticker in
                self.tickers}
            return self.data

    def _plot(self, df, mas: bool=True, constants: bool=False, **kwargs
        ) ->None:
        if constants:
            chart_lines = npAppend(npArange(-5, 6, 1), npArange(-100, 110, 10))
            df.ta.constants(True, chart_lines)
            df.ta.constants(False, npArray([-60, -40, 40, 60]))
            if self.verbose:
        ...
        if ta.Imports['matplotlib']:
            ...
            _colors = kwargs.pop('figsize', ['black', 'green', 'orange',
                'red', 'maroon'])
            ...
            if mas:
                ...
                price = df[col]
            if _kind is None:
                price.tail(_last).plot(figsize=_figsize, color=_colors,
                    linewidth=2, title=_title, grid=_grid, alpha=_alpha)
            else:
                print(f'[X] Plot kind not implemented')
                return

    def load(self, ticker: str=None, tf: str=None, index: str='date', drop:
        list=[], plot: bool=False, **kwargs) ->pd.DataFrame:
    ...
    def __repr__(self) ->str:
        s = (
            f"Watch(name='{self.name}', ds_name='{self.ds_name}', tickers[{len(self.tickers)}]='{', '.join(self.tickers)}', tf='{self.tf}', strategy[{self.strategy.total_ta()}]='{self.strategy.name}'"
            )
...
cudf_colors = cudf.DataFrame({
    'aliases': ['BkGy', 'BkSv', 'BkPr', 'BkBl', 'FcLi', 'GrRd', 'GyBk', 'GyBl', 'GyOr', 'GyPr', 'GySv', 'RdGr', 'SvGy', 'BkGrRd', 'BkBlPr', 'GrOrRd', 'RdOrGr', 'BkGrOrRd', 'BkGrOrRdMr', 'bbands', 'kc'],
    'colors': [['black', 'gray'], ['black', 'silver'], ['black', 'purple'], ['black', 'blue'], ['fuchsia', 'lime'], ['green', 'red'], ['gray', 'black'], ['gray', 'blue'], ['gray', 'orange'], ['gray', 'purple'], ['gray', 'silver'], ['red', 'green'], ['silver', 'gray'], ['black', 'green', 'red'], ['black', 'blue', 'purple'], ['green', 'orange', 'red'], ['red', 'orange', 'green'], ['black', 'green', 'orange', 'red'], ['black', 'green', 'orange', 'red', 'maroon'], ['blue', 'navy', 'blue'], ['purple', 'fuchsia', 'purple']]
})

def colors(colors: str=None, default: str='GrRd'):
    if colors in cudf_colors['aliases'].values:
        return cudf_colors[cudf_colors['aliases'] == colors]['colors'].values[0]
    return cudf_colors[cudf_colors['aliases'] == default]['colors'].values[0]