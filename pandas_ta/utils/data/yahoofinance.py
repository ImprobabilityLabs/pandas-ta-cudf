    ...
    * Chart History:
    ...
    if Imports['yfinance'] and ticker is not None:
        ...
        if kind in _all + ['info'] or verbose:
            ...
            ci_header = (
                f"({ticker_info['shortName']}) [{ticker_info['symbol']}]")
            ...
            if 'address1' in ticker_info and len(ticker_info['address1']):
                ...
                if 'city' in ticker_info and len(ticker_info['city']
                    ) and 'state' in ticker_info and len(ticker_info['state']
                    ) and 'zip' in ticker_info and len(ticker_info['zip']
                    ) and 'country' in ticker_info and len(ticker_info[
                    'country']):
                    print(
            ...
            if 'website' in ticker_info and len(ticker_info['website']):
                ...
                if 'fullTimeEmployees' in ticker_info:
                    s += (f"FT Employees: {ticker_info['fullTimeEmployees']:,}"
                        .rjust(40))
                print(s)
            elif 'fullTimeEmployees' in ticker_info:
                print(f"FT Employees: {ticker_info['fullTimeEmployees']:,}")
            if 'companyOfficers' in ticker_info and len(ticker_info[
                'companyOfficers']):
                print(
            ...
            if 'sector' in ticker_info and len(ticker_info['sector']
                ...
                print(f'Sector | Industry'.ljust(29),
                    f"{ticker_info['sector']} | {ticker_info['industry']}".
                    rjust(50))
            print('\n====  Market Information   ' + div)
            _category = (f" | {ticker_info['category']}" if 'category' in
                ticker_info and ticker_info['category'] is not None else '')
            print(
            ...
            if 'marketCap' in ticker_info and ticker_info['marketCap'
                ...
                print(f'Market Cap.'.ljust(39),
                    f"{ticker_info['marketCap']:,} ({ticker_info['marketCap'] / 1000000:,.2f} MM)"
                    .rjust(40))
            if 'navPrice' in ticker_info and ticker_info['navPrice'
                ] is not None or 'yield' in ticker_info and ticker_info['yield'
                ] is not None:
                print(f'NAV | Yield'.ljust(39),
                    f"{ticker_info['navPrice']} | {100 * ticker_info['yield']:.4f}%"
                    .rjust(40))
            if 'sharesOutstanding' in ticker_info and ticker_info[
                'sharesOutstanding'
                ] is not None and 'floatShares' in ticker_info and ticker_info[
                'floatShares'] is not None:
                print(f'Shares Outstanding | Float'.ljust(39),
            ...
            if ('sharesShort' in ticker_info and 'shortRatio' in
                ticker_info and ticker_info['sharesShort'] is not None and 
                ticker_info['shortRatio'] is not None):
                print(f'Shares Short | Ratio'.ljust(39),
                    f"{ticker_info['sharesShort']:,} | {ticker_info['shortRatio']:,}"
                    .rjust(40))
            if ('shortPercentOfFloat' in ticker_info and ticker_info[
                'shortPercentOfFloat'] is not None and 
                'sharesShortPriorMonth' in ticker_info and ticker_info[
                'sharesShortPriorMonth'] is not None):
                print(f'Short % of Float | Short prior Month'.ljust(39),
            ...
            if ('heldPercentInstitutions' in ticker_info and ticker_info[
                'heldPercentInstitutions'] is not None or 
                'heldPercentInsiders' in ticker_info and ticker_info[
                'heldPercentInsiders'] is not None):
                print(f'Insiders % | Institution %'.ljust(39),
                    f"{100 * ticker_info['heldPercentInsiders']:.4f}% | {100 * ticker_info['heldPercentInstitutions']:.4f}%"
                    .rjust(40))
            print()
            if 'bookValue' in ticker_info and ticker_info['bookValue'
                ] is not None or 'priceToBook' in ticker_info and ticker_info[
                'priceToBook'
                ] is not None or 'pegRatio' in ticker_info and ticker_info[
                'pegRatio'] is not None:
                print(f'Book Value | Price to Book | Peg Ratio'.ljust(39),
                    f"{ticker_info['priceToBook']} | {ticker_info['priceToBook']} | {ticker_info['pegRatio']}"
                    .rjust(40))
            if 'forwardPE' in ticker_info and ticker_info['forwardPE'
                ] is not None:
                print(f'Forward PE'.ljust(39),
                    f"{ticker_info['forwardPE']}".rjust(40))
            if 'forwardEps' in ticker_info and ticker_info['forwardEps'
                ] is not None or 'trailingEps' in ticker_info and ticker_info[
                'trailingEps'] is not None:
                print(f'Forward EPS | Trailing EPS'.ljust(39),
            ...
            if ('enterpriseToRevenue' in ticker_info and ticker_info[
                'enterpriseToRevenue'] is not None or 'enterpriseToEbitda' in
                ticker_info and ticker_info['enterpriseToEbitda'] is not None):
                print(f'Enterprise to Revenue | to EBITDA'.ljust(39),
            ...
            if 'profitMargins' in ticker_info and ticker_info['profitMargins'
                ] is not None:
            ...
            _o, _h, _l, _c, _v = ticker_info['open'], ticker_info['dayHigh'
                ], ticker_info['dayLow'], ticker_info['regularMarketPrice'
                ], ticker_info['regularMarketVolume']
            print(f'Open High Low | Close'.ljust(39),
                f'{_o:.4f}  {_o:.4f}  {_l:.4f} | {_c:.4f}'.rjust(40))
            print(f'HL2 | HLC3 | OHLC4 | C - OHLC4'.ljust(39),
                f'{0.5 * (_h + _l):.4f}, {(_h + _l + _c) / 3.0:.4f}, {0.25 * (_o + _h + _l + _c):.4f}, {_c - 0.25 * (_o + _h + _l + _c):.4f}'
                .rjust(40))
            print(f'Change (%)'.ljust(39),
                f"{_c - ticker_info['previousClose']:.4f} ({100 * (_c / ticker_info['previousClose'] - 1):.4f}%)"
                .rjust(40))
            if 'bid' in ticker_info and ticker_info['bid'
                ] is not None and 'bidSize' in ticker_info and ticker_info[
                'bidSize'
                ] is not None and 'ask' in ticker_info and ticker_info['ask'
                ] is not None and 'askSize' in ticker_info and ticker_info[
                'askSize'] is not None:
                print(f'Bid | Ask | Spread'.ljust(39),
            ...
            print(
                f"{ticker_info['volume']:,} | {_v:,} | {ticker_info['averageVolume']:,} ({ticker_info['averageDailyVolume10Day']:,})"
                .rjust(80))
            print()
            if '52WeekChange' in ticker_info and ticker_info['52WeekChange'
                ] is not None:
            ...
            if 'SandP52WeekChange' in ticker_info and ticker_info[
                ...
                print(f'52Wk % Change vs S&P500'.ljust(39),
                    f"{100 * ticker_info['SandP52WeekChange']:.4f}%".rjust(40))
            if ('fiftyTwoWeekHigh' in ticker_info and 'fiftyTwoWeekLow' in
                ticker_info and 'previousClose' in ticker_info):
                print(f'52Wk Range (% from 52Wk Low)'.ljust(39),
            ...
            if avg50 and avg200:
                print(f'SMA 50 | SMA 200'.ljust(39),
            ...
            if 'beta' in ticker_info and ticker_info['beta'
                ] is not None and 'beta3Year' in ticker_info and ticker_info[
                'beta3Year'] is not None:
                print(f'Beta | 3Yr'.ljust(39),
            ...
            if ('threeYearAverageReturn' in ticker_info and ticker_info[
                'threeYearAverageReturn'] is not None and 
                'fiveYearAverageReturn' in ticker_info and ticker_info[
                'fiveYearAverageReturn'] is not None):
                print(f'Avg. Return 3Yr | 5Yr'.ljust(39),
            ...
            if not dividends.empty or not splits.empty:
                ...
                if ('dividendRate' in ticker_info and ticker_info[
                    'dividendRate'] is not None and 'dividendYield' in
                    ticker_info and ticker_info['dividendYield'] is not
                    None and 'payoutRatio' in ticker_info and ticker_info[
                    'payoutRatio'] is not None):
                    print(f'Rate | Yield | Payout Ratio'.ljust(39),
                        f"{ticker_info['dividendRate']} | {100 * ticker_info['dividendYield']:.4f}% | {ticker_info['payoutRatio']}"
                        .rjust(40))
                if ('trailingAnnualDividendRate' in ticker_info and 
                    ticker_info['trailingAnnualDividendRate'] is not None and
                    'trailingAnnualDividendYield' in ticker_info and 
                    ticker_info['trailingAnnualDividendYield'] is not None):
                    print(f'Trailing Annual Dividend Rate | Yield'.ljust(40
                        ),
...
"""
        ...
        if kind in _all + ['earnings', 'earn']:
            earndf = yfd.earnings
            if not earndf.empty:
                earndf['Revenue'] = earndf.apply(lambda x:
                    f"{x['Revenue']:,}", axis=1)
                earndf['Earnings'] = earndf.apply(lambda x:
                    f"{x['Earnings']:,}", axis=1)
                if kind not in _all:
                    print(f"\n{ticker_info['symbol']}")
                print('\n====  Earnings             ' + div + f'\n{earndf}')
        if kind in _all + ['sustainability', 'sus', 'esg']:
            ...
            if susdf is not None:
                ...
                susdf.rename(index=_camelCase2Title, errors='ignore',
                    inplace=True)
...
"""
                ...
                if opt_date not in opt_expirations:
                    ...
                    option_columns = ['Contract', 'Last Trade', 'Strike',
                        'Price', 'Bid', 'Ask', 'Change', 'Percent Change',
                        'Volume', 'OI', 'IV', 'ITM', 'Size', 'Currency']
