...
class TestMomentum(TestCase):
    ...
    def test_inertia(self):
        ...
        result = pandas_ta.inertia(self.close, self.high, self.low, refined
            =True)
        self.assertIsInstance(result, Series)
    ...
    def test_macd(self):
        ...
        try:
            ...
            expecteddf = DataFrame({'MACD_12_26_9': expected[0],
                'MACDh_12_26_9': expected[2], 'MACDs_12_26_9': expected[1]})
            pdt.assert_frame_equal(result, expecteddf)
        except AssertionError:
            try:
                macd_corr = pandas_ta.utils.df_error_analysis(result.iloc[:,
                    0], expecteddf.iloc[:, 0], col=CORRELATION)
                self.assertGreater(macd_corr, CORRELATION_THRESHOLD)
            except Exception as ex:
                error_analysis(result.iloc[:, 0], CORRELATION, ex)
            try:
                history_corr = pandas_ta.utils.df_error_analysis(result.
                    iloc[:, 1], expecteddf.iloc[:, 1], col=CORRELATION)
                self.assertGreater(history_corr, CORRELATION_THRESHOLD)
            except Exception as ex:
                error_analysis(result.iloc[:, 1], CORRELATION, ex, newline=
                    False)
            try:
                signal_corr = pandas_ta.utils.df_error_analysis(result.iloc
                    [:, 2], expecteddf.iloc[:, 2], col=CORRELATION)
                self.assertGreater(signal_corr, CORRELATION_THRESHOLD)
            except Exception as ex:
                error_analysis(result.iloc[:, 2], CORRELATION, ex, newline=
                    False)
    ...
    def test_ppo(self):
        ...
        try:
            ...
            pdt.assert_series_equal(result['PPO_12_26_9'], expected,
                check_names=False)
        except AssertionError:
            try:
                corr = pandas_ta.utils.df_error_analysis(result[
                    'PPO_12_26_9'], expected, col=CORRELATION)
                self.assertGreater(corr, CORRELATION_THRESHOLD)
            except Exception as ex:
    ...
    def test_squeeze(self):
        ...
        result = pandas_ta.squeeze(self.high, self.low, self.close, tr=
            False, lazybear=True)
        self.assertIsInstance(result, DataFrame)
    ...
    def test_squeeze_pro(self):
        ...
        result = pandas_ta.squeeze_pro(self.high, self.low, self.close, tr=
            False)
        self.assertIsInstance(result, DataFrame)
        self.assertEqual(result.name, 'SQZPROhlr_20_2.0_20_2_1.5_1')
        result = pandas_ta.squeeze_pro(self.high, self.low, self.close, 20,
            2, 20, 3, 2, 1)
        self.assertIsInstance(result, DataFrame)
    ...
    def test_stoch(self):
        ...
        try:
            expected = tal.STOCH(self.high, self.low, self.close, 14, 3, 0,
                3, 0)
            expecteddf = DataFrame({'STOCHk_14_3_0_3_0': expected[0],
                'STOCHd_14_3_0_3': expected[1]})
            pdt.assert_frame_equal(result, expecteddf)
        except AssertionError:
            try:
                stochk_corr = pandas_ta.utils.df_error_analysis(result.iloc
                    [:, 0], expecteddf.iloc[:, 0], col=CORRELATION)
                self.assertGreater(stochk_corr, CORRELATION_THRESHOLD)
            except Exception as ex:
                error_analysis(result.iloc[:, 0], CORRELATION, ex)
            try:
                stochd_corr = pandas_ta.utils.df_error_analysis(result.iloc
                    [:, 1], expecteddf.iloc[:, 1], col=CORRELATION)
    ...
    def test_stochrsi(self):
        ...
        try:
            ...
            expecteddf = DataFrame({'STOCHRSIk_14_14_0_3': expected[0],
                'STOCHRSId_14_14_3_0': expected[1]})
            pdt.assert_frame_equal(result, expecteddf)
        except AssertionError:
            try:
                stochrsid_corr = pandas_ta.utils.df_error_analysis(result.
                    iloc[:, 0], expecteddf.iloc[:, 1], col=CORRELATION)
                self.assertGreater(stochrsid_corr, CORRELATION_THRESHOLD)
            except Exception as ex:
                error_analysis(result.iloc[:, 0], CORRELATION, ex, newline=
                    False)
