...
class TestVolatility(TestCase):
    ...
    def test_bbands(self):
        ...
        try:
            ...
            expecteddf = DataFrame({'BBU_5_2.0': expected[0], 'BBM_5_2.0':
                expected[1], 'BBL_5_2.0': expected[2]})
            pdt.assert_frame_equal(result, expecteddf)
        except AssertionError:
            try:
                bbl_corr = pandas_ta.utils.df_error_analysis(result.iloc[:,
                    0], expecteddf.iloc[:, 0], col=CORRELATION)
                self.assertGreater(bbl_corr, CORRELATION_THRESHOLD)
            except Exception as ex:
                error_analysis(result.iloc[:, 0], CORRELATION, ex)
            try:
                bbm_corr = pandas_ta.utils.df_error_analysis(result.iloc[:,
                ...
                error_analysis(result.iloc[:, 1], CORRELATION, ex, newline=
                    False)
            try:
                bbu_corr = pandas_ta.utils.df_error_analysis(result.iloc[:,
    ...
    def test_donchian(self):
        ...
        result = pandas_ta.donchian(self.high, self.low, lower_length=20,
            upper_length=5)
        self.assertIsInstance(result, DataFrame)
